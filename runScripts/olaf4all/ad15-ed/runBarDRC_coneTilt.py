...
if eagleFlag:
    save_dir                = '/projects/bar/mchetan/projects/downwind/ad15-ed-Group/'
else:
...
fastBatch.FAST_runDirectory = os.path.join(save_dir + 'outputs' + os.sep + 'drc')
...
pitch_ref   = [ 0.    ,  0.    ,  0.    ,  0.    ,  0.    ,  0.8669,  2.81  ,
        6.178 ,  8.604 , 10.4   , 12.27  , 13.84  , 15.25  , 16.81  ,
       18.06  , 19.34  , 20.71  , 21.79  , 35.01  , 35.27  , 35.29  ,
       35.44  , 35.62  ] # Pitch values in deg
...
# Cone and tilt mods
case_inputs[("ElastoDyn","PreCone(1)")]     = {'vals':[2.0, 0.0, -2.0, 2.0, 0.0], 'group':2}
case_inputs[("ElastoDyn","PreCone(2)")]     = case_inputs[("ElastoDyn","PreCone(1)")]
case_inputs[("ElastoDyn","PreCone(3)")]     = case_inputs[("ElastoDyn","PreCone(1)")]
case_inputs[("ElastoDyn","ShftTilt")]     = {'vals':[5, 5, 5, 2, 2], 'group':2}
